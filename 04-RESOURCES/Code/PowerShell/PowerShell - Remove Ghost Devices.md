...
description: PowerShell - Remove Ghost Devices
tags:
  - Type/Code
  - Status/Complete
  - Topic/PowerShell
...
aliases:
  - Remove Ghost Devices with PowerShell
---

### Custom [[C#]] Class for the `SetupApi`
...
Function Remove-GhostDevices {
    ...
    If ($WhatIf) {
        Write-Verbose "WhatIf is enabled. No devices will be removed." -Verbose
        $removeDevices = $false
    }
    else {
        $removeDevices = $true
        ...
        if ($removeDevices -eq $true) {
            if ($InstallState -eq $False) {
                if ($matchFilter -eq $False) {
                    Write-Verbose "Attempting to removing device $FriendlyName" -Verbose
                    $removeObj = New-Object -TypeName System.Object
                    ...
                    if ([Win32.SetupApi]::SetupDiRemoveDevice($devs, [ref]$devInfo)) {
                        Write-Verbose "Removed device $FriendlyName" -Verbose
                    }
                    else {
                        Write-Verbose "Failed to remove device $FriendlyName" -Verbose
                    }
                }
                else {
                    Write-Verbose "Filter matched. Skipping $FriendlyName" -Verbose
                }
            }
        }
        $devcount++
    ...
    if ($removeDevices -eq $true) {
        Write-Verbose "Removed devices:" -Verbose
        $removeArray | Sort-Object -Property FriendlyName | Format-Table
        Write-Verbose "Total removed devices: $($removeArray.Count)" -Verbose
        return $removeArray | Out-Null
    }
    else {
        Write-Verbose "Devices that would be removed:" -Verbose
        $array | Where-Object -FilterScript { $_.InstallState -eq $False } | Sort-Object -Property FriendlyName | Format-Table
        Write-Verbose "Total devices that would be removed: $($array.Count)" -Verbose
        return $array | Where-Object -FilterScript { $_.InstallState -eq $False }
    }
}
```
